...
#
#   ./scripts/explain-pushdown.sh "SELECT count(*) FROM ch.station_status"
...
if printf '%s' "$REMOTE" | grep -qiE '\bGROUP BY\b|\bcount\(|\bsum\(|\bavg\(|\bmin\(|\bmax\('; then
    ...
    echo
    echo "The usual cause is a local table in the join. Both bike.stations and"
    echo "bike.station_status have to be replicated for the join to stay remote."
    echo
    printf '%s\n' "$REMOTE" | mask
fi
